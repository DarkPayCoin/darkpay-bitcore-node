...
The Darkpay Service is a Node.js interface to [Darkpay Core](https://github.com/DarkPayCoin/darkpay-core) for querying information about the darkpay block chain. It will manage starting and stopping `darkpayd` or connect to several running `darkpayd` processes. It uses an instance of a Darkpay with additional indexes enabled for querying information about addresses and blocks. Results are cached for performance and there are several additional API methods added for common queries.

## Configuration

The default configuration will include a "spawn" configuration in "darkpayd". This defines the location of the block chain database and the location of the `darkpayd` daemon executable. The below configuration points to a local clone of `darkpay`, and will start `darkpayd` automatically with your Node.js application.
...
It's also possible to connect to separately managed `darkpayd` processes with round-robin quering, for example:
